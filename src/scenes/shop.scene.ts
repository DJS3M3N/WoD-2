...
shopScene.enter(async (ctx) => {
	...
	for (let offer of offers) {
		console.log(offer);
		const item = await ItemService.getItem(offer.item_id + 1);
	...
	formState.message_id = (
		await ctx.replyWithPhoto(
			"https://webshop.abo.fi/wp-content/uploads/woocommerce-placeholder-600x600.png",
			{
				reply_markup: {
					inline_keyboard: [
	...
	shopScene.action("buy_with_money", async (ctx) => {
		...
		const result = await ShopService.buyOfferWithMoney(
			offers_displayed[formState.num - 1].id,
			ctx.from.id
		);
		if (result) {
			ctx.reply(
				...
				Markup.inlineKeyboard([
					Markup.button.callback("Вернуться к меню", "open_menu"),
				])
			);
		} else {
			ctx.reply(
				"Не удалось купить предмет, возможно на вашем счету недостаточно средств!",
				Markup.inlineKeyboard([
					Markup.button.callback("Вернуться к меню", "open_menu"),
				])
			);
	...
	shopScene.action("buy_with_rm_currency", async (ctx) => {
		ctx.editMessageReplyMarkup({ inline_keyboard: [] });
		const result = await ShopService.buyOfferWithRMCurrency(
			offers_displayed[formState.num - 1].id,
			ctx.from.id
		);
		if (result) {
			ctx.reply(
				"Предмет куплен",
				Markup.inlineKeyboard([
					Markup.button.callback("Вернуться в меню", "open_menu"),
				])
			);
		} else {
			ctx.reply(
	...
	function showOffer(
		...
		const inline_keyboard: InlineKeyboardMarkup = {
			inline_keyboard: [
				[
					{ text: "<<< (ещё " + (num - 1) + ")", callback_data: "prev_item" },
					{
						text: "(ещё " + (offers.length - num) + ") >>>",
		...
		ctx.telegram.editMessageMedia(
			...
			undefined,
			{
				...
				caption:
					...
					offers[num - 1].power +
					"</b>\n💎 Цены: <b>" +
					(offers[num - 1].currency_type === "both" ||
					offers[num - 1].currency_type === "money"
						? offers[num - 1].price + "💰"
						: "") +
					(offers[num - 1].currency_type === "both" ||
					offers[num - 1].currency_type === "rm_currency"
						? " " + offers[num - 1].rm_price + " 🟡"
						: "") +
					"</b>\n👚 Надевается в слот: <b>" +
					offers[num - 1].slot +
					"</b>\n📃 Описание: <b>" +
					offers[num - 1].description +
					"</b>",
				parse_mode: "HTML",
			},
